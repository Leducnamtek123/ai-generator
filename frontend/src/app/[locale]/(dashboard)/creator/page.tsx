    ...
    <div className="min-h-screen bg-background text-foreground pt-8 px-8 max-w-[1600px] mx-auto">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-2">
          <h1 className="text-3xl font-semibold tracking-tight">{t('workspace.creator')}</h1>
          <p className="max-w-2xl text-sm text-muted-foreground">
            {tDashboard('availableToolRoutes')}
          </p>
        </div>
        ...
        <div className="relative w-full max-w-md">
          ...
          <Input
            ...
            className="w-full rounded-full border-border bg-card py-2.5 pl-9 pr-4 text-sm"
          />
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-border bg-card p-5">
          <p className="text-3xl font-semibold">{ALL_TOOLS_LIST.length}</p>
          <p className="mt-1 text-sm text-muted-foreground">{tDashboard('availableToolRoutes')}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-5">
          <p className="text-3xl font-semibold">{socialItems.length}</p>
          <p className="mt-1 text-sm text-muted-foreground">{tDashboard('publishingInboxCalendarDashboard')}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-5">
          <p className="text-3xl font-semibold">{CREATOR_TOOL_HIGHLIGHTS.length}</p>
          <p className="mt-1 text-sm text-muted-foreground">{tDashboard('featuredShortcuts')}</p>
        </div>
      </div>
      ...
      <div className="mt-10 grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-8">
        {CREATOR_TOOL_HIGHLIGHTS.map((tool) => (
          <ToolCard
            key={tool.label}
            icon={tool.icon}
            label={translateLayoutLabel(t, tool.label)}
            href={tool.href}
            isNew={tool.isNew}
            color={tool.color}
          />
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold leading-tight">{translateLayoutLabel(t, item.label)}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{tDashboard('openPage')}</p>
                  </div>
                </div>
                {item.isNew && (
            ...
            <section key={category.id} className="space-y-4">
              <div className="flex items-center justify-between gap-4">
                <h3 className="flex items-center gap-2 text-lg font-semibold text-foreground">
                  ...
                  {translateLayoutLabel(t, category.label)}
                </h3>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex min-w-0 items-start gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="font-semibold leading-tight">{translateLayoutLabel(t, tool.label)}</p>
                            <p className="mt-1 text-xs text-muted-foreground">{tDashboard('openTool')}</p>
                          </div>
                        </div>
                        {tool.isNew && (
                  <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">
