...
import { Copy, Upload, Download, Sparkles, Loader2, RotateCcw, Folder, Shuffle, Grid3X3 } from 'lucide-react';
...
const mockVariations = [
    'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=400&h=400&fit=crop',
    'https://images.unsplash.com/photo-1620641788421-7a1c342ea42e?q=80&w=400&h=400&fit=crop',
    'https://images.unsplash.com/photo-1633167606207-d840b5070fc2?q=80&w=400&h=400&fit=crop',
    'https://images.unsplash.com/photo-1605721911519-3dfeb3be25e7?q=80&w=400&h=400&fit=crop',
        ...
        <div className="h-full bg-background text-foreground flex overflow-hidden">
            <div className="w-[320px] border-r border-border flex flex-col shrink-0 bg-background">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {/* Upload */}
                    <div onClick={() => fileInputRef.current?.click()} className="group relative aspect-square rounded-2xl bg-muted border-2 border-dashed border-border hover:border-primary/30 transition-all cursor-pointer overflow-hidden flex flex-col items-center justify-center gap-3">
                        {uploadedImage ? (<img src={uploadedImage} alt="Source" className="w-full h-full object-cover" />) : (
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-1.5">
                            {variationTypes.map((t) => (
                                <button key={t.id} onClick={() => setVariationType(t.id)} className={cn("w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border transition-all text-left", variationType === t.id ? "bg-accent border-primary/20" : "bg-card border-border hover:border-border/80")}>
                                    <div className={cn("w-4 h-4 rounded-full border-2 flex items-center justify-center", variationType === t.id ? "border-primary" : "border-muted-foreground/30")}>
                                        {variationType === t.id && <div className="w-2 h-2 rounded-full bg-primary" />}
                                    </div>
                    ...
                    <div className="space-y-5">
                        <div className="space-y-3">
                            <div className="flex items-center justify-between"><Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.1em]">Count</Label><span className="text-[11px] font-mono">{count}</span></div>
                            <Slider min={2} max={8} step={1} value={[count]} onValueChange={([v]) => setCount(v)} />
                        </div>
                        <div className="space-y-3">
                            <div className="flex items-center justify-between"><Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.1em]">Variation Strength</Label><span className="text-[11px] font-mono">{strength}%</span></div>
                            <Slider min={10} max={100} step={5} value={[strength]} onValueChange={([v]) => setStrength(v)} />
                        </div>
                        <div className="space-y-3">
                    ...
                    <div className="space-y-3">
                        ...
                        <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Guide the variation direction..." className="w-full h-20 bg-card border border-border rounded-xl p-3 text-xs resize-none outline-none focus:ring-2 focus:ring-ring placeholder:text-muted-foreground" />
                    </div>
                </div>
                <div className="p-4 border-t border-border space-y-3">
                    <div className="flex items-center justify-between text-xs text-muted-foreground px-1"><span>Cost:</span><span className="font-medium text-foreground">{count} Credits</span></div>
                    <Button onClick={handleGenerate} disabled={isGenerating || !uploadedImage} className="w-full h-12 font-bold rounded-xl gap-2">
                        {isGenerating ? (<><Loader2 className="w-5 h-5 animate-spin" /> Generating...</>) : (<><Shuffle className="w-5 h-5" /> Generate Variations</>)}
                    </Button>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                {results.length > 0 && (
                    <div className="h-14 px-6 border-b border-border flex items-center justify-between shrink-0">
                        <span className="text-sm font-medium">{results.length} variations</span>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {isGenerating ? (
                        <div className="flex flex-col items-center justify-center h-full gap-4"><div className="relative"><div className="w-16 h-16 rounded-full border-4 border-muted border-t-primary animate-spin" /><Shuffle className="w-6 h-6 text-muted-foreground absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" /></div><p className="text-sm text-muted-foreground animate-pulse">Creating {count} variations...</p></div>
                    ) : results.length > 0 ? (
                        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
                            {/* Source Image */}
                            <div className="relative">
                                <div className="aspect-square rounded-2xl border-2 border-primary/30 overflow-hidden shadow-lg"><img src={uploadedImage!} alt="Source" className="w-full h-full object-cover" /></div>
                            ...
                            {results.map((url, i) => (
                                <div key={i} className="group relative cursor-pointer" onClick={() => setSelectedResult(selectedResult === i ? null : i)}>
                                    <div className={cn("aspect-square rounded-2xl border overflow-hidden shadow-lg transition-all", selectedResult === i ? "border-primary ring-2 ring-primary/20 scale-[1.02]" : "border-border hover:border-primary/30")}>
