...
import { workspaceApi, type Workspace, type UpdateWorkspaceData } from '@/services/workspaceApi';
import { useWorkspaceStore } from '@/stores/workspace-store';
import { WorkspaceSettingsSkeleton } from '@/components/common/loading-skeletons';
import {
...
type WorkspaceSettingsAction =
    | { type: 'setWorkspace'; workspace: Workspace | null }
    ...
    | { type: 'setForm'; form: UpdateWorkspaceData }
    | { type: 'patchForm'; patch: UpdateWorkspaceData };
...
function reducer(state: WorkspaceSettingsState, action: WorkspaceSettingsAction): WorkspaceSettingsState {
    switch (action.type) {
        case 'setWorkspace':
...
export default function OrgSettingsPage() {
    ...
    const { currentWorkspace, setCurrentWorkspace, hasPermission } = useWorkspaceStore();
    ...
    const canUpdate = hasPermission('update', 'Workspace');
    const canDelete = hasPermission('delete', 'Workspace');

    const loadWorkspace = useCallback(async () => {
        try {
            const data = await workspaceApi.get(slug);
            dispatch({ type: 'setWorkspace', workspace: data });
            dispatch({
                type: 'setForm',
                form: {
                    name: data.name,
            ...
            dispatch({ type: 'setError', error: 'Failed to load workspace' });
        }
    ...
    const handleSave = async () => {
        ...
        try {
            const updated = await workspaceApi.update(slug, state.form);
            dispatch({ type: 'setWorkspace', workspace: updated });
            if (currentWorkspace?.id === updated.id) {
                setCurrentWorkspace(updated);
            }
            dispatch({ type: 'setSuccess', success: 'Workspace updated successfully' });
        ...
        <div className="max-w-2xl mx-auto py-8 px-4 sm:px-6">
            <div className="mb-8">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                    Manage your workspace&apos;s configuration and details
                </p>
            </div>
            ...
            <div className="space-y-6">
                <div className="bg-card border border-border rounded-xl p-6 space-y-5">
                    ...
                    <div className="flex items-center gap-2 p-3 bg-muted/50 rounded-lg text-xs text-muted-foreground">
                        ...
                        Slug: <code className="font-mono bg-background px-1.5 py-0.5 rounded">{state.workspace?.slug}</code>
                    </div>
                </div>
                ...
                {canDelete && (
                    <div className="bg-destructive/5 border border-destructive/20 rounded-xl p-6 space-y-4">
                        ...
                        <p className="text-sm text-muted-foreground">Deleting this workspace will permanently remove all projects, members, and data.</p>
