...
export function NodeToolbar({ nodeId, onRun, onRunLocal, runDisabled, onDelete, onDuplicate, onReference, onReplace, onSettings }: NodeToolbarProps) {
    ...
    return (
        <div className="absolute -top-14 left-1/2 z-50 flex -translate-x-1/2 items-center gap-1 rounded-lg border border-border/70 bg-card/95 px-1.5 py-1 shadow-[0_16px_40px_rgba(0,0,0,0.24)] backdrop-blur-xl animate-in fade-in slide-in-from-bottom-2 duration-200">
            {onRun && (
...
function ToolbarButton({ children, onClick, className, tooltip, disabled, tooltipEnabled = true }: { children: React.ReactNode, onClick?: () => void, className?: string, tooltip?: string, disabled?: boolean, tooltipEnabled?: boolean }) {
    return (
        <button
            ...
            disabled={disabled}
            className={cn(
                "relative group flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-foreground disabled:cursor-not-allowed disabled:opacity-30",
                className
            )}
        >
            {children}
            {tooltipEnabled && tooltip && (
                <span className="absolute bottom-full left-1/2 mb-2 -translate-x-1/2 whitespace-nowrap rounded-md border border-border/70 bg-background px-2 py-1 text-[10px] text-foreground opacity-0 transition-opacity group-hover:opacity-100 pointer-events-none">
                    {tooltip}
                </span>
            )}
        </button>
