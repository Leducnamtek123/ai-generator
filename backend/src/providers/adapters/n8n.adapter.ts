...
export class N8nAdapter implements AIProvider {
    ...
    async generateImage(prompt: string, options?: ImageOptions): Promise<GenerationResult> {
        const webhookUrl = this.configService.get<string>('providers.imageGeneration.n8n.webhookUrl');
        ...
        try {
            const response = await this.httpService.axiosRef.post(webhookUrl, {
                ...
                callbackUrl: `${this.configService.get('app.backendDomain')}/api/v1/generations/callback`,
            });

            this.logger.log(`Image generation triggered via n8n: ${response.data?.executionId}`);
    ...
    async generateVideo(prompt: string, options?: VideoOptions): Promise<GenerationResult> {
        const webhookUrl = this.configService.get<string>('providers.videoGeneration.n8n.webhookUrl');
    ...
    async upscaleImage(imageUrl: string, options?: UpscaleOptions): Promise<GenerationResult> {
        const webhookUrl = this.configService.get<string>('providers.upscaler.n8n.webhookUrl');
    ...
    async enhancePrompt(prompt: string, style?: string): Promise<string> {
        const webhookUrl = this.configService.get<string>('providers.promptEnhancer.n8n.webhookUrl');
