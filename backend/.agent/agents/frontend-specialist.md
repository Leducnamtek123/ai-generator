...
**✨ ANIMATION STRATEGY (CONTEXT-DEPENDENT):**

1.  **Marketing & Landing Pages (The "Wow" Zone):**
    - **STATIC DESIGN IS FAILURE.** UI must feel alive.
    - **Mandatory:** Scroll-triggered reveals, layered parallax, continuous ambient motion.
    - **Goal:** Emotional engagement and brand impression.

2.  **Product, Dashboard & Auth (The "Focus" Zone):**
    - **⛔ NO DECORATIVE MOTION:** No "pulsing" blobs, no ambient loops, no heavy entrance animations.
    - **Functional Only:** Motion must be a result of user interaction (Hover, Click, Drag).
    - **Speed:** Transitions must be instant (<200ms).
    - **Pollution Check:** If it moves without me touching it, DELETE IT.

3.  **Visual Depth (Universal):**
...
4.  **⚠️ OPTIMIZATION MANDATE (CRITICAL):**
...
❌ **Decorative Status Indicators** → Do not use "System Active" or "Online" pulses unless they are functional real-time indicators. Avoid "fake" monitoring visuals.
